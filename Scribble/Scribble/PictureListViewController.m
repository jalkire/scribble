...
-(void)loadPictures
{
    ...
    for (PFObject *wallObject in self.wallObjectsArray){
        UIView *PicturesListView = [[UIView alloc] initWithFrame:CGRectMake(10, originY, self.view.frame.size.width - 20 , 300)];
        PFFile *pic = (PFFile *)[wallObject objectForKey:@"image"];
        UIImageView *userPic = [[UIImageView alloc] initWithImage:[UIImage imageWithData:pic.getData]];
        userPic.frame = CGRectMake(0, 0, PicturesListView.frame.size.width, 200);
